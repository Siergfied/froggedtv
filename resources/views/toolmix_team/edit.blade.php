    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
            <div class="bg-white p-4 shadow sm:rounded-lg sm:p-8">
                <form method="POST" action="{{ route('toolmixTeam.update') }}">
                    @csrf
                    @method('patch')

                    <p> {{ Auth::user()->toolmixTeam }} </p>
                    ...
                    <div>
                        ...
                        <x-text-input id="mmr_min" name="mmr_min" type="number" min="0" class="mt-1 block w-full"
                            :value="old('mmr_min', $toolmixTeam->mmr_min)" autofocus autocomplete="name" />
                    ...
                    <div>
                        ...
                        <textarea id="commentaire" name="commentaire"
                            class="mt-2 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('commentaire', $toolmixTeam->commentaire) }}</textarea>
